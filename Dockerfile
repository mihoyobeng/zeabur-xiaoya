# 基础镜像：选用稳定的Ubuntu 22.04（X86架构，适配Zeabur默认环境）
FROM ubuntu:22.04

# 环境变量配置：规避交互式安装提示，预设核心路径与版本（无需后续手动配置）
ENV DEBIAN_FRONTEND=noninteractive
ENV XIAOYA_HOME=/opt/xiaoya
ENV MEDIA_DIR=/opt/xiaoya/media  # 小雅媒体库目录（绝对路径，可按需修改）
ENV EMBY_VERSION=4.9.0.42        # 预设Emby指定版本
ENV EMBY_IMAGE=amilys/embyserver # Zeabur X86架构适配的Emby镜像
ENV CRON_ENABLE=yes              # 启用定时爬虫功能
...
EXPOSE 5678  # Alist核心访问端口（必暴露）
EXPOSE 8096  # Emby核心访问端口（必暴露）
EXPOSE 22    # 备用终端端口（可选）
EXPOSE 443   # 备用HTTPS端口（可选）
